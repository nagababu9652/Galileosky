...
object Galileosky extends GalileoskyMapV2 {
  ...
  val galileoskyCase: String => String = (Stri: String) => {
    ...
    while(tag == "01"){
      ...
      // for records in packet
      while((pointer - tagdataStartPnt ) < taglen * 2){
        breakable{
          // for record
          while ((pointer - tagdataStartPnt ) < taglen * 2) {
            ...
            if (tag == "FE") {
              ...
              pointer = ptrInc(2)
              // for extended tag in record
              while (pointer - recordDataStart < BigInt(LittleEdianform(ext_len),16) * 2) {
